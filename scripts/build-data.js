...
const csvFilename = 'immunisation-5in1.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?indicator ?measure ?value
            WHERE {
                ?s qb:dataSet data:dtp-pol-hib-uptake-by-24-months ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   <http://statistics.gov.scot/def/dimension/indicator(dtpPolHibUptake)> ?i ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?z rdfs:label ?zone .
